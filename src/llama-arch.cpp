...
static const std::map<llm_tensor, llm_tensor_info> LLM_TENSOR_INFOS = {
    ...
    // NextN/MTP tensors exist on the final decoder layer(s), but their GGUF
    // names are still block-scoped (`blk.%d.nextn.*`), so they must be treated
    // as repeating layer tensors by the loader.
