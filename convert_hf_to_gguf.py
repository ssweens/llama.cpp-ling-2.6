...
class BailingMoeV2_5Model(BailingMoeV2Model):
    ...
    def set_gguf_parameters(self):
        ...
        # MLA dimensions (DeepSeek-V3 style with q-LoRA). For the absorbed MLA
        # KV cache, the runtime key vector is [kv_lora_rank + qk_rope_head_dim]
        # (compressed latent + rotary side-channel). The decompressed logical MLA
        # dimensions remain qk_nope+qk_rope and v_head_dim.
        self.gguf_writer.add_q_lora_rank(hparams["q_lora_rank"])
        self.gguf_writer.add_kv_lora_rank(hparams["kv_lora_rank"])
